"""Run candidate v5 on both RETIRED external development sets.

This is DEVELOPMENT / DIAGNOSTIC ONLY.

The runner executes external-v1 and external-v2 in one prediction pass before
any scoring. It imports candidate v5 directly from _candidates/ and never
...
- imports the exact candidate-v5 source by SHA;
- performs no scoring;
- refuses to overwrite output artifacts.

Run from backend/:
    uv run python -m py_compile scripts/run_external_retired_dev_predictions_candidate_v5.py
    uv run python -m scripts.run_external_retired_dev_predictions_candidate_v5

Inputs:
    tests/external_questions_blind_v1.json
    tests/external_questions_blind_v2.json
    _candidates/ask_two_stage_evidence_then_answer_candidate_v5.py

Outputs:
    tests/external_predictions_dev_v1_candidate_v5.json
    tests/external_predictions_dev_v2_candidate_v5.json
"""
...
def load_candidate() -> ModuleType:
    module_name = "waypoint_candidate_v5_dev"
    ...
    for required in (
        ...
        if not hasattr(module, required):
            raise RuntimeError(
                f"Candidate v5 is missing required symbol: {required}"
            )
...
async def run_dataset(
    *,
    ...
    print()
    print(
        f"Running candidate v5 on {dataset_name} "
    ...
    output = {
        ...
        "candidate_name": (
            "two_stage_evidence_then_answer_v5"
        ),
        ...
        "candidate_entrypoint": (
            "_candidates/"
            "ask_two_stage_evidence_then_answer_candidate_v5.py:ask"
        ),
        "model_calls_per_question": 2,
    ...
    if verify.get("model_calls_per_question") != 2:
        raise RuntimeError(
            f"{dataset_name}: unexpected model-call metadata."
        )
...
async def main() -> None:
    ...
    print(
        "Waypoint candidate-v5 retired external "
    ...
    try:
        ...
        print()
        print(
            "Candidate-v5 retired development "
            "prediction run: PASS"
        )
