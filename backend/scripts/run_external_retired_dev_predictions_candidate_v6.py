"""Run candidate v6 on both RETIRED external development sets.

This is DEVELOPMENT / DIAGNOSTIC ONLY.

The runner executes external-v1 and external-v2 in one prediction pass before
any scoring. It imports candidate v6 directly from _candidates/ and never
replaces production app/api/routes/ask.py.
...
- verifies the historical prediction artifacts and frozen production runtime;
- imports the exact candidate-v6 source by SHA;
- performs no scoring;
- refuses to overwrite output artifacts.

Run from backend/:
    uv run python -m py_compile scripts/run_external_retired_dev_predictions_candidate_v6.py
    uv run python -m scripts.run_external_retired_dev_predictions_candidate_v6

Inputs:
    tests/external_questions_blind_v1.json
    tests/external_questions_blind_v2.json
    _candidates/ask_support_then_authority_then_answer_candidate_v6.py

Outputs:
    tests/external_predictions_dev_v1_candidate_v6.json
    tests/external_predictions_dev_v2_candidate_v6.json
"""
...
def load_candidate() -> ModuleType:
    module_name = "waypoint_candidate_v6_dev"
    ...
    # Register before execution so Pydantic can resolve postponed annotations
    # from this dynamically imported module.
    ...
    for required in (
        ...
        if not hasattr(module, required):
            raise RuntimeError(
                f"Candidate v6 is missing required symbol: {required}"
            )
...
async def run_dataset(
    *,
    ...
    print()
    print(
        f"Running candidate v6 on {dataset_name} "
        ...
        for index, item in enumerate(questions, start=1):
            try:
                ...
                errors.append(
                    {
                        ...
                        "question": item["question"],
                        "error_type": "candidate_contract_error",
                ...
                print(
                    ...
                    f"{item['case_id']} ERROR 502: {exc.detail}"
                )
                continue
    ...
    if len(predictions) + len(errors) != len(questions):
        raise RuntimeError(
            f"{dataset_name}: attempted-case count mismatch."
        )
    ...
    output = {
        ...
        "candidate_name": (
            "support_then_authority_then_answer_v6"
        ),
        ...
        "candidate_entrypoint": (
            "_candidates/"
            "ask_support_then_authority_then_answer_candidate_v6.py:ask"
        ),
        "model_call_contract": {
        ...
        "observed_successful_prediction_distribution": {
            ...
            "scope": "successful_predictions_only",
        },
        "candidate_errors_are_scoring_failures": True,
    ...
    if verify.get("attempted_count") != len(questions):
        raise RuntimeError(
            f"{dataset_name}: saved attempted count verification failed."
        )

    if (
        verify.get("prediction_count", 0)
        + verify.get("error_count", 0)
        != len(questions)
    ):
        raise RuntimeError(
            f"{dataset_name}: saved result count verification failed."
        )
    ...
    if verify.get(
        "candidate_errors_are_scoring_failures"
    ) is not True:
        raise RuntimeError(
            f"{dataset_name}: candidate-error scoring contract changed."
        )

    call_contract = verify.get("model_call_contract", {})
    if call_contract != {
        "support_sufficient": 2,
        "support_insufficient": 3,
    }:
        raise RuntimeError(
            f"{dataset_name}: unexpected model-call contract metadata."
        )

    call_distribution = verify.get(
        "observed_successful_prediction_distribution",
        {},
    )
    if (
        call_distribution.get("two_calls", 0)
        + call_distribution.get("three_calls", 0)
        != len(predictions)
    ):
        raise RuntimeError(
            f"{dataset_name}: successful-call distribution mismatch."
        )

    print(
        f"{dataset_name} successful predictions: "
        f"{len(predictions)}/{len(questions)}"
    )
    print(
        f"{dataset_name} explicit candidate errors: "
        f"{len(errors)}/{len(questions)}"
    )
    print(
...
async def main() -> None:
    ...
    print(
        "Waypoint candidate-v6 retired external "
    ...
    try:
        ...
        print()
        print(
            "Candidate-v6 retired development "
