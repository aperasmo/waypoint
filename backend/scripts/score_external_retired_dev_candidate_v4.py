"""Score candidate v4 on both RETIRED external development sets.

This is DEVELOPMENT / DIAGNOSTIC ONLY.

It compares:
- frozen candidate v2 historical predictions;
- candidate v4 factorised-evidence predictions;
...
Run from backend/:
    uv run python -m py_compile scripts/score_external_retired_dev_candidate_v4.py
    uv run python -m scripts.score_external_retired_dev_candidate_v4
"""
    ...
    *,
    ...
    if candidate == "v4":
        if payload.get(
            ...
            raise RuntimeError("Candidate-v4 SHA linkage changed.")

        if payload.get(
            "production_runtime_sha256"
        ) != EXPECTED_V2_RUNTIME_SHA256:
            raise RuntimeError("Candidate-v4 production linkage changed.")

        if payload.get("production_runtime_replaced") is not False:
            raise RuntimeError(
                "Candidate-v4 artifact claims production replacement."
            )
...
def comparison(
    ...
    for case_id, gold_item in gold.items():
        ...
        v4_status = v4[case_id]["evidence_status"]

        base_ok = base_status == gold_status
        v4_ok = v4_status == gold_status
        ...
        if not base_ok and v4_ok:
            gains.append(item)
        elif base_ok and not v4_ok:
            regressions.append(item)
        elif not base_ok and not v4_ok and base_status != v4_status:
            wrong_to_different_wrong.append(item)
        elif base_ok and v4_ok:
...
def print_score(
    ...
    *,
    ...
    print(
        f"Candidate v4: {v4_score['correct']}/{total} "
        f"({pct(v4_score['correct'], total)})"
    )
    delta = v4_score["correct"] - baseline_score["correct"]
    ...
    for status in STATUSES:
        gold_n = v4_score["gold_counts"][status]
        base_n = baseline_score["correct_counts"][status]
        v4_n = v4_score["correct_counts"][status]
        print(
            f"  {status:<26}"
            f"v2 {base_n:>2}/{gold_n:<2} {pct(base_n, gold_n):>7}"
            f"   v4 {v4_n:>2}/{gold_n:<2} {pct(v4_n, gold_n):>7}"
        )

    print()
    print("Candidate-v4 predicted distribution")
    for status in STATUSES:
        print(
            f"  {status:<26}"
            f"{v4_score['predicted_counts'][status]}"
        )

    print()
    print("Candidate-v4 confusion matrix")
    print(
        "gold / predicted               sufficient  corpus_gap    external"
    )
    for gold_status in STATUSES:
        row = v4_score["confusion"][gold_status]
        print(
            f"{gold_status:<32}"
            f"{row['sufficient']:>10}"
            f"{row['corpus_gap']:>12}"
    ...
    print(
        ...
        f"{v4_score['false_sufficient']}/"
        f"{v4_score['nonsufficient_gold']} "
        f"({pct(v4_score['false_sufficient'], v4_score['nonsufficient_gold'])})"
    )
    ...
    print(
        f"  Any expected: "
        f"{v4_score['any_expected']}/{v4_score['sufficient_count']} "
        f"({pct(v4_score['any_expected'], v4_score['sufficient_count'])})"
    )
    print(
        f"  All expected: "
        f"{v4_score['all_expected']}/{v4_score['sufficient_count']} "
        f"({pct(v4_score['all_expected'], v4_score['sufficient_count'])})"
    )
    print(
        f"  No citations: "
        f"{v4_score['no_citations']}/{v4_score['sufficient_count']}"
    )
    ...
    if show_clusters:
        ...
        print(
            ...
            f"{100.0 * v4_score['cluster_macro']:.1f}%"
        )
        print(
            f"Fully correct source clusters: "
            f"{v4_score['full_clusters']}/20"
        )
...
def main() -> None:
    ...
    v1_v4 = prediction_map(
        load_json(V1_V4_PATH),
        expected_schema=(
            "waypoint-external-predictions-dev-v1-candidate-v4"
        ),
        expected_count=51,
        candidate="v4",
    )
    v2_v4 = prediction_map(
        load_json(V2_V4_PATH),
        expected_schema=(
            "waypoint-external-predictions-dev-v2-candidate-v4"
        ),
        expected_count=60,
        candidate="v4",
    )
    ...
    validate_alignment(v1_gold, v1_v4, "v1 candidate-v4")
    validate_alignment(v2_gold, v2_base, "v2 baseline")
    validate_alignment(v2_gold, v2_v4, "v2 candidate-v4")
    ...
    v1_comp = comparison(v1_gold, v1_base, v1_v4)
    v2_comp = comparison(v2_gold, v2_base, v2_v4)

    combined_gold = {**v1_gold, **v2_gold}
    combined_base = {**v1_base, **v2_base}
    combined_v4 = {**v1_v4, **v2_v4}
    ...
    print("Waypoint candidate-v4 retired development score")
    print("=" * 48)
    print(
        f"Candidate v4 SHA256:      "
        f"{EXPECTED_V4_CANDIDATE_SHA256}"
    )
    print(
    ...
    print()
    print("Candidate-v4 retired development scoring: PASS")
