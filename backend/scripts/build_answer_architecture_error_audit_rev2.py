...
Output:
    tests/answer_architecture_error_audit_v2_v5_v6_rev2.json
"""
...
def evidence_availability(
    ...
    if gold_item["evidence_status"] == "sufficient":
        ...
        any_expected_top5 = bool(expected_top5)
        any_expected_top10 = bool(expected_top10)
        all_expected_top5 = expected <= set(top5)
        all_expected_top10 = expected <= set(top10)

        if all_expected_top5:
            complete_support_availability = (
                "all_listed_expected_sections_present_top5"
            )
        elif all_expected_top10:
            complete_support_availability = (
                "all_listed_expected_sections_present_top10_only"
            )
        elif any_expected_top5:
            complete_support_availability = (
                "some_but_not_all_listed_expected_sections_present_top5"
            )
        elif any_expected_top10:
            complete_support_availability = (
        ...
        support_availability = (
            "any_expected_section_present_top5"
            if any_expected_top5
            else (
                "any_expected_section_present_top10_only"
                if any_expected_top10
                else "no_expected_section_present_top10"
            )
        )
    else:
        support_availability = "not_applicable_non_sufficient_gold"
        complete_support_availability = (
            "not_applicable_non_sufficient_gold"
        )
        any_expected_top5 = False
        any_expected_top10 = False
        all_expected_top5 = False
        all_expected_top10 = False

    return {
        "support_availability": support_availability,
        "complete_listed_expected_section_coverage": (
            complete_support_availability
        ),
        "any_expected_section_in_top5": any_expected_top5,
        "any_expected_section_in_top10": any_expected_top10,
        "all_listed_expected_sections_in_top5": all_expected_top5,
        "all_listed_expected_sections_in_top10": all_expected_top10,
...
def main() -> None:
    ...
    sufficient_complete_coverage = Counter()
    ...
    for case_id, gold_item in gold.items():
        ...
        if gold_item["evidence_status"] == "sufficient":
            ...
            sufficient_complete_coverage[
                availability[
                    "complete_listed_expected_section_coverage"
                ]
            ] += 1

            if availability["any_expected_section_in_top5"]:
    ...
    if sum(sufficient_complete_coverage.values()) != 25:
        raise RuntimeError(
            "Expected 25 sufficient complete-coverage cases."
        )
    ...
    summary = {
        ...
        "sufficient_evidence_availability_any_expected_section": dict(
            sufficient_availability
        ),
        "sufficient_evidence_availability_all_listed_expected_sections": dict(
            sufficient_complete_coverage
        ),
    ...
    findings = {
        "retrieval_support_check": {
            "definition": (
                "For gold-sufficient cases only, compare frozen expected_sections "
                "against the pre-existing adjudication packet retrieval "
                "top-5/top-10. Report both whether ANY listed expected "
                "section is present and whether ALL listed expected "
                "sections are present. This is a factual retrieval "
                "availability diagnostic, not a claim that every listed "
                "section is independently necessary."
            ),
            "any_expected_section_present_top5": (
                sufficient_availability[
                    "any_expected_section_present_top5"
            ...
            "no_expected_section_present_top10": (
                sufficient_availability[
                    "no_expected_section_present_top10"
                ]
            ),
    ...
    print("-" * 68)
    print(
        "At least one expected section in top-5: "
        f"{sufficient_availability['any_expected_section_present_top5']}/25"
    )
    print(
        "At least one expected section in top-10 only: "
        f"{sufficient_availability['any_expected_section_present_top10_only']}/25"
    )
    print(
        "No expected section in top-10: "
        f"{sufficient_availability['no_expected_section_present_top10']}/25"
    )
    print()
    print("Complete listed expected-section coverage")
    print("-" * 68)
    print(
        "All listed expected sections in top-5: "
        f"{sufficient_complete_coverage['all_listed_expected_sections_present_top5']}/25"
    )
    print(
        "All listed expected sections in top-10 only: "
        f"{sufficient_complete_coverage['all_listed_expected_sections_present_top10_only']}/25"
    )
    print(
        "Some but not all listed expected sections in top-5: "
        f"{sufficient_complete_coverage['some_but_not_all_listed_expected_sections_present_top5']}/25"
    )
    print(
        "Some but not all listed expected sections in top-10 only: "
        f"{sufficient_complete_coverage['some_but_not_all_listed_expected_sections_present_top10_only']}/25"
    )
    print(
        "No listed expected sections in top-10: "
        f"{sufficient_complete_coverage['no_listed_expected_sections_present_top10']}/25"
    )
    print()
