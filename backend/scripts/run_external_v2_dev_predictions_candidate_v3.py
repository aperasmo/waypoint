"""Run answer candidate v3 on retired external holdout v2 as DEVELOPMENT data.

This is NOT an untouched holdout run. External v2 has already been inspected
and used to inform candidate v3.
...
- performs no scoring;
- writes a new development prediction artifact;
- refuses to overwrite existing prediction artifacts.

Run from backend/:
    uv run python -m py_compile scripts/run_external_v2_dev_predictions_candidate_v3.py
    uv run python -m scripts.run_external_v2_dev_predictions_candidate_v3

Input:
    tests/external_questions_blind_v2.json

Output:
    tests/external_predictions_dev_v2_candidate_v3.json
"""
...
OUTPUT_PATH = (
    BACKEND_DIR
    / "tests"
    / "external_predictions_dev_v2_candidate_v3.json"
)
...
EXPECTED_HISTORICAL_FIRST_PREDICTIONS_SHA256 = (
    "BCC045922577E84AA89CBBE19587E56C"
    "634ABEB119F9476191B050FB2459493D"
)

HISTORICAL_FIRST_PREDICTIONS_PATH = (
...
def validate_blind(payload: dict) -> list[dict]:
    ...
    if payload.get("status") != EXPECTED_INPUT_STATUS:
        raise RuntimeError(
            f"Unexpected blind status: {payload.get('status')!r}"
        )

    questions = payload.get("questions")
    if not isinstance(questions, list) or len(questions) != 60:
        raise RuntimeError(
            "Expected exactly 60 blind v2 questions."
        )

    if payload.get("question_count") != 60:
        raise RuntimeError(
            "Blind metadata question_count is not 60."
        )
...
async def main() -> None:
    ...
    require_sha(
        INPUT_PATH,
        EXPECTED_BLIND_SHA256,
        "Retired external-v2 blind input",
    )
    ...
    print("Waypoint candidate-v3 external-v2 DEVELOPMENT run")
    print("=" * 49)
    ...
    print(
        ...
        f"{sha256(HISTORICAL_FIRST_PREDICTIONS_PATH)}"
    )
    print()
    print("External-v2 status:        DEVELOPMENT / DIAGNOSTIC ONLY")
    print("Gold file opened:          NO")
    print("Expected sections loaded:  NO")
    print("Scoring performed:         NO")
    print("Production ask pipeline:   YES")
    print()
    print("Running candidate v3 on retired external-v2...")
    ...
    try:
        ...
        if len(predictions) != 60:
            raise RuntimeError(
                f"Expected 60 predictions, got {len(predictions)}."
            )

        output = {
            "schema": "waypoint-external-predictions-dev-v2-candidate-v3",
        ...
        if verify.get("prediction_count") != 60:
            raise RuntimeError(
        ...
        print("Predictions completed:     60/60")
        print()
        print("External-v2 status:        DEVELOPMENT / DIAGNOSTIC ONLY")
        print("Gold file opened:          NO")
        print("Expected sections loaded:  NO")
        ...
        print()
        print("Candidate-v3 external-v2 development run: PASS")
